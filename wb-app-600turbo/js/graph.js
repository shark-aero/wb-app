...
// VARS 
//weight
const ballastWeight = 4;
const oxygenWeight = 4;
...
var limiterBoxPilotSmall = document.getElementById('limiter-box-pilot-small');
...
var idCautionTextSmallPilot = document.getElementById('caution-text-small-pilot');
...
function updateFigure() {
    ...
    // ballast position
    ballastMoment = switchBallastPosition * ballastFrontMoment + (1 - switchBallastPosition) * ballastRearMoment;

    // oxygen position
    oxygenMoment = switchOxygenPosition * oxygenWeight * armOxygen;

    // pilot local arm
    localArmPilot = Math.min(armPilotHeavy, coef_a * pilotWeight + coef_b);

    // takeoff weight
    var weightTO = emptyWeight + pilotWeight + passengerWeight + baggageWeight + ballastWeight + fuelWeight + switchOxygenPosition * oxygenWeight;
    // zero fuel weight
    var weightZF = emptyWeight + pilotWeight + passengerWeight + baggageWeight + ballastWeight + switchOxygenPosition * oxygenWeight;

    // takeoff moment
    var momentTO = startMoment + fuelWeight * armFuel + pilotWeight * localArmPilot +
        passengerWeight * armPassenger + baggageWeight * armBaggage + ballastMoment + oxygenMoment;
    // zero fuel moment
    ...
    if (pilotWeight < 60) {
        limiterBoxPilotSmall.style.background = '#ff000059'
    } else {
        limiterBoxPilotSmall.style.background = 'var(--third)'
    }
    ...
    // if Pilot < 60kg
    if (flagCaution[4]) {
        idCautionTextSmallPilot.style.display = "block"
    } else {
        idCautionTextSmallPilot.style.display = "none"
    }
...
function checkCautions(pilotWeight, passengerWeight, baggageWeight, fuelVolume) {
    ...
    var flagBaggage = false;
    var flagFuel = false;
    var flagTwoOccupants = false;
    var flagMaxOccupants = false;
    var flagSmallPilot = false;
    ...
    if (pilotWeight + passengerWeight > 200) {
        flagTwoOccupants = true;
    }

    // flag two occupants warning
    if ((pilotWeight > 110) || (passengerWeight > 110)) {
        flagMaxOccupants = true;
    }

    // flag small pilot warning
    if (pilotWeight < 60) {
        flagSmallPilot = true;
    }

    return [flagBaggage, flagFuel, flagTwoOccupants, flagMaxOccupants, flagSmallPilot]
}